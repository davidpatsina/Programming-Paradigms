...
def findOptimalAlignment(strand1, strand2, optimals):
	optimal = {}
    
    # if one of the two strands is empty, then there is only
	# one possible alignment, and of course it's optimal
	if len(strand1) == 0:
        optimal['strand1'] = (len(strand2) * ' ')
        optimal['strand2'] = strand2
        optimal['score'] = (len(strand2) * -2)
        return optimal
	if len(strand2) == 0:
        optimal['strand1'] = strand1
        optimal["strand2"] = " " * len(strand1)
        optimal["score"] = len(strand1) * -2
        return optimal
    
    key = ((strand1 + '_') + strand2)
        if optimals.has_key(key):
        return deepcopy(optimals[key])


	# There's the scenario where the two leading bases of
	# each strand are forced to align, regardless of whether or not
	# they actually match.
	bestWith = findOptimalAlignment(strand1[1:], strand2[1:], optimals)
	...
	# opposite scenario
	bestWithout = findOptimalAlignment(strand1[1:], strand2, optimals)
    ...
    optimals[key] = deepcopy(best)
	return best deepcopy(best)
