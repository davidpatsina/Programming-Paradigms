...
(define (intersection-points circle-ls)
  (define (intersections-with-first-point)
    (apply append (map (lambda (pt) 
                         (intersect (car circle-ls) pt)) 
                       (cdr circle-ls))))
  (if (< (length circle-ls) 2)
    '()
    (append (intersections-with-first-point) 
            (intersection-points (cdr circle-ls)))))

(define (distance-product point lst)
  (let ((lst-minus-point (remove point lst)))
    (apply * (map (lambda (point2) (dist point point2)) lst-minus-point))))
...
(define (sort-points points)
  (quicksort points (lambda (point1 point2) (< (car point1) (car point2)))))


(define (clumped-points points)
  (let ((sorted-points (map cadr (sort-points (rate-points points)))))
    (prefix-of-list sorted-points (floor (/ (length sorted-points) 2)))))

(define (average l)
  (/ (apply + l) (length l)))

(define (average-point points)
(let ((avg-point (list (average (map car points))
...
(define (where-am-i distances stars)
  (quicksort (map best-estimate (all-guesses distances stars)) 
             (lambda (point1 point2) (< (car point1) (car point2)))))
;;; --------------------------------------------------------------------------------------
;;; 				!!!!!!! TESTING PART !!!!!!!
